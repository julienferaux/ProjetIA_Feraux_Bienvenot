...
class GameState:
    ...
    def move_piece(self, start_row, start_col, end_row, end_col):
        ...
        if (self.board[start_row][start_col] != 1) & \
            ...
            raise ValueError(
                "There is no piece to move in the starting position :" + str(self.board[start_row][start_col]))
        if self.board[end_row][end_col] != 0:
            raise ValueError("The destination position is not empty : start :", self.board[start_row][start_col],
            ...
            try:
                ...
                if (self.board[vx][vy] != 0 and self.board[vx2][vy2] != 0):
                    ...
                    if couleurv1 != maCouleur:
                        if ((vx2, vy2) in [(0, 0), (0, taille - 1), (taille - 1, 0), (taille - 1, taille - 1),
                                           (int(taille / 2), int(taille / 2))]):
                            self.board[vx][vy] = 0
                        else:
